...
class EvccPageIndicator {
    ...
    public function setCenterAngle( angle as Number ) {
        if( angle < 0 || angle > 360 ) {
            throw new InvalidValueException( "setCenterAngle: degree " + angle + " is not in valid range between 0 and 360." );
        }
        _centerAngle = angle;
    }
    ...
    public function setDotDistanceAngle( angle as Number ) {
        if( angle < 1 || angle > 90 ) {
            throw new InvalidValueException( "setDotDistanceAngle: degree " + angle + " is not in valid range between 1 and 90." );
        }
    ...
    function orbitXY( centerX as Number, centerY as Number, degree as Float, radius as Float ) as [Number,Number] {
        if( degree < 0 || degree > 360 ) {
            throw new InvalidValueException( "orbitXY: degree " + degree + " is not in valid range between 0 and 360." );
        }
