...
(:exclForSitesOne :exclForViewPreRenderingDisabled) public class EvccStateRequestRegistry {
    ...
    // Sets the active site (information is only passed on to the timer, not needed in this class)
    public static function setActiveSite( activeSite as Number ) as Void { 
        ...
        ( _stateRequestTimer as EvccMultiStateRequestsTimer ).setActiveSite( activeSite );
    }
    ...
    public static function start( activeSite as Number ) as Void {
        ...
        _stateRequestTimer = new EvccMultiStateRequestsTimer( _stateRequests, activeSite );
    }
